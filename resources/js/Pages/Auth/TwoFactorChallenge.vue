    ...
    <jet-authentication-card>
        ...
        <div class="mb-4 text-sm text-gray-600">
            <template v-if="! recovery">
                {{ __('auth.twoFactor.description') }}
            </template>

            <template v-else>
                {{ __('auth.twoFactor.recoveryDescription') }}
            </template>
        </div>
        ...
        <form @submit.prevent="submit">
            <div v-if="! recovery">
                <jet-label for="code" :value="__('auth.twoFactor.code')" />
            ...
            <div v-else>
                <jet-label for="recovery_code" :value="__('auth.twoFactor.recoveryCode')" />
            ...
            <div class="flex items-center justify-end mt-4">
                <button type="button" class="text-sm text-gray-600 hover:text-gray-900 underline cursor-pointer" @click.prevent="toggleRecovery">
                    <template v-if="! recovery">
                        {{ __('auth.twoFactor.codeBtn') }}
                    </template>

                    <template v-else>
                        {{ __('auth.twoFactor.recoveryCodeBtn') }}
                    </template>
                </button>

                <jet-button class="ml-4" :class="{ 'opacity-25': form.processing }" :disabled="form.processing">
                    {{ __('auth.twoFactor.login') }}
                </jet-button>
            </div>
        </form>
    </jet-authentication-card>
